# 코딩 규칙 (Coding Rules)

> 작성일: 2025-12-30  
> 최종 업데이트: 2025-12-30  
> 목적: 코드 품질 및 유지보수성 향상을 위한 필수 규칙

## 🎯 핵심 원칙

1. **TypeScript Only** 🔥
2. **Maximum Code Reuse** 🔥
3. **TDD Required** 🔥
4. **File Size Limit** 🔥 (NEW!)
5. **System Structure Integrity** 🔥

---

## 📏 파일 크기 관리 규칙 (엄격 적용)

### 🔴 필수 규칙

#### 1. 700행 초과 시 무조건 분리
- 파일이 **700행을 초과**하면 **즉시 모듈화** 필수
- 예외 없음 - "나중에 할게요" 불가
- 커밋 전 반드시 확인

#### 2. 새 기능은 별도 파일로
- 새로운 기능 추가 시 **반드시 별도 컴포넌트/모듈**로 작성
- 기존 파일에 "일단 추가" 금지
- 처음부터 올바른 위치에 생성

### 📊 라인 수 기준

| 범위 | 분류 | 조치 |
|------|------|------|
| 0-150줄 | 소형 | 통합 검토 |
| 150-500줄 | **최적** | ✅ 목표 범위 |
| 500-700줄 | 허용 | 주의 필요 |
| **700줄+** | **분리필수** | 🔴 즉시 분리 |

### ✅ 모듈화 체크리스트

커밋 전 반드시 확인:

```bash
# 1. 700행 초과 파일 확인
Get-ChildItem -Recurse -Filter "*.tsx","*.ts" | Where-Object { 
  (Get-Content $_.FullName | Measure-Object -Line).Lines -gt 700 
} | Select-Object Name, @{Name="Lines";Expression={(Get-Content $_.FullName | Measure-Object -Line).Lines}}

# 2. 코드 인덱스 업데이트
.\scripts\code-index-manager.ps1 -Generate -Optimize
```

- [ ] 700행 초과 파일 없음
- [ ] 신규 기능은 별도 파일로 생성
- [ ] 코드 인덱스 업데이트 완료
- [ ] 관련 파일 같은 폴더에 배치
- [ ] 폴더 깊이 4단계 이하 유지

---

## 🏗️ 모듈화 원칙

### 1. 응집력 우선
- 논리적으로 관련된 코드는 함께 유지
- 700행 이내를 목표로 하되, 응집력이 높으면 유지 가능
- 예: 복잡한 단일 기능 로직 (600-700줄)

### 2. 미로 방지
- 폴더 깊이: 최대 4단계
- 관련 파일: 같은 폴더에 배치
- 인덱스 문서: 주기적 갱신

### 3. 명확한 책임 분리
- 하나의 파일 = 하나의 명확한 책임
- UI 컴포넌트 vs 비즈니스 로직 분리
- 공통 로직은 유틸리티로 추출

---

## 📁 디렉토리 구조 가이드

### 권장 구조

```
src/app/pfmea/worksheet/
├── page.tsx              # 메인 페이지 (700줄 이하 유지)
├── constants.ts          # 상수 정의
├── hooks/                # 커스텀 훅
│   ├── index.ts
│   └── useWorksheetState.ts
├── utils/                # 유틸리티 함수
│   ├── index.ts
│   ├── types.ts
│   ├── failure-link-grouping.ts
│   ├── row-merge-logic.ts
│   └── process-grouping.ts
├── tabs/                 # 탭 컴포넌트
│   ├── analysis/         # 분석 탭
│   │   ├── structure/
│   │   ├── function/
│   │   └── failure/
│   ├── evaluation/       # 평가 탭
│   │   ├── EvalStructureTab.tsx
│   │   ├── EvalFunctionTab.tsx
│   │   └── EvalFailureTab.tsx
│   └── shared/           # 공용 컴포넌트
└── components/           # 로컬 컴포넌트
```

### 금지 사항

❌ 모든 로직을 `page.tsx`에 작성  
❌ 깊은 폴더 구조 (5단계 이상)  
❌ 비슷한 이름의 파일을 여러 곳에 분산  
❌ 700행 넘는 파일 방치

---

## 🔄 리팩토링 시나리오

### 시나리오 1: 기존 파일이 700행 초과

```typescript
// AS-IS: page.tsx (2,747줄) ❌
export default function Page() {
  // 모든 로직이 여기에...
  return <div>...</div>
}

// TO-BE: 분리 후
// page.tsx (700줄 이하) ✅
import { EvalStructureTab, EvalFunctionTab } from './tabs/evaluation';
export default function Page() {
  return <div>...</div>
}

// tabs/evaluation/EvalStructureTab.tsx (116줄) ✅
export default function EvalStructureTab() { ... }

// tabs/evaluation/EvalFunctionTab.tsx (195줄) ✅
export default function EvalFunctionTab() { ... }
```

### 시나리오 2: 새 기능 추가

```typescript
// ❌ 잘못된 방법: 기존 파일에 추가
// page.tsx (이미 600줄)
function Page() {
  // ... 기존 로직 600줄
  
  // 새로운 "역전개" 기능 추가 (200줄) → 총 800줄 초과!
  const reverseFeature = () => { ... }
  
  return <div>...</div>
}

// ✅ 올바른 방법: 별도 파일로 생성
// tabs/extended/reverse/ReverseFMEATab.tsx (200줄)
export default function ReverseFMEATab() {
  // 역전개 기능 로직
  return <div>...</div>
}

// page.tsx (610줄) - 10줄만 추가
import ReverseFMEATab from './tabs/extended/reverse/ReverseFMEATab';
function Page() {
  // ... 기존 로직 600줄
  if (tab === 'reverse') return <ReverseFMEATab />;
  return <div>...</div>
}
```

---

## 🚨 위반 시 조치

### 레벨 1: 경고 (Warning)
- **조건**: 파일이 700-800줄
- **조치**: 다음 커밋 전 분리 필요
- **알림**: PR 코멘트

### 레벨 2: 차단 (Blocking)
- **조건**: 파일이 800줄 초과
- **조치**: CI 빌드 실패
- **요구사항**: 분리 완료 후 재시도

### 레벨 3: 롤백 (Rollback)
- **조건**: 반복적 위반 (3회 이상)
- **조치**: 커밋 롤백 및 재작성
- **요구사항**: 리팩토링 플랜 제출

---

## 📚 관련 문서

- `docs/REFACTORING_MASTER_PLAN.md` - 리팩토링 마스터 플랜
- `docs/CODE_OPTIMIZATION_REVIEW.md` - 코드 최적화 검토
- `docs/MODULARIZATION_LESSONS_LEARNED.md` - 모듈화 교훈
- `docs/CODE_INDEX.md` - 코드 인덱스 (수동)
- `docs/CODE_INDEX_AUTO.md` - 코드 인덱스 (자동생성)

---

## 🛠️ 자동화 도구

### 코드 인덱스 관리

```powershell
# 코드 인덱스 생성 및 최적화 제안
.\scripts\code-index-manager.ps1 -Generate -Optimize

# 700행 초과 파일 자동 검색
.\scripts\find-large-files.ps1 -Threshold 700
```

### Pre-commit Hook (예정)

```bash
# .git/hooks/pre-commit
#!/bin/sh
# 700행 초과 파일 체크
large_files=$(find src -name "*.ts" -o -name "*.tsx" | xargs wc -l | awk '$1 > 700 {print $2}')
if [ ! -z "$large_files" ]; then
  echo "❌ Error: Files exceeding 700 lines found:"
  echo "$large_files"
  echo "Please split these files before committing."
  exit 1
fi
```

---

## ✅ 성공 사례

### v1.8.0-v1.8.1 리팩토링 (2025-12-30)

**Before**:
- `page.tsx`: 2,747줄 ❌

**After**:
- `page.tsx`: 2,264줄 ✅ (483줄 감소)
- `utils/`: 5개 파일 생성 (200줄)
- `tabs/evaluation/`: 3개 파일 생성 (423줄)

**결과**:
- ✅ 중복 코드 제거
- ✅ 테스트 용이성 향상
- ✅ 유지보수성 개선
- ✅ 700행 규칙 준수

---

## 📋 요약

1. **700행 초과 = 즉시 분리** 🔴
2. **새 기능 = 새 파일** 🟢
3. **커밋 전 체크 필수** ✅
4. **응집력 우선, 미로 방지** 🎯
5. **자동화 도구 활용** 🛠️

**Remember**: "나중에 할게요" → "지금 당장 하세요!" 💪


