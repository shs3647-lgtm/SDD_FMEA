...
| 파일 | 시작 | 현재 | 목표 | 감소 |
|------|------|------|------|------|
| `page.tsx` | 2,747줄 | 2,264줄 | ~700줄 | 483줄 (18%) |
| `FailureLinkTab.tsx` | 1,102줄 | 1,047줄 | ~400줄 | 55줄 (5%) |
| `schema.ts` | 997줄 | 997줄 | ~800줄 | 0줄 (0%) |

**현재까지 감소:** 538줄  
**총 목표 감소:** ~1,750줄  
**진행률:** 31%  
**신규 파일 생성:** 8개 (확장 기능 포함)
...
### ✅ Step 1: 공통 유틸리티 함수 추출 (완료)

**목표**: 중복 로직을 재사용 가능한 유틸리티 함수로 추출

**소요 시간**: 2시간  
**위험도**: 🟢 낮음  
**실제 효과**: 중복 코드 200줄 감소 (page.tsx 145줄, FailureLinkTab.tsx 55줄)

**완료 날짜**: 2025-12-30  
**커밋**: `refactor: Step 1 - extract common utilities for failure links`
...
- [x] `tabs/evaluation/` 디렉토리 생성
- [x] `EvalStructureTab.tsx` 작성 (116줄)
- [x] `EvalFunctionTab.tsx` 작성 (195줄)
- [x] `EvalFailureTab.tsx` 작성 (112줄)
- [x] `constants.ts` 색상 정의 확장
- [x] `page.tsx`에서 컴포넌트 분리 적용 (365줄 감소)
- [x] 빌드 테스트: 성공
- [ ] 수동 테스트: eval-structure 탭 정상 동작
- [ ] 수동 테스트: eval-function 탭 정상 동작
- [ ] 수동 테스트: eval-failure 탭 정상 동작
- [x] 커밋: "refactor: Step 2 part1 - eval-structure tab split"

**완료 날짜**: 2025-12-30  
**실제 소요 시간**: 1.5시간  
**실제 효과**: page.tsx 365줄 감소  
**위험도**: 🟢 낮음 (타입 에러 1건 해결)
...
| **Step 8** | ✅ 완료 | 2025-12-30 | 2025-12-30 | 3시간 | **우측 패널 플러그인 아키텍처 - TreePanel, ParetoChart 구현** |
