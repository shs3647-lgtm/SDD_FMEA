    ...
    allIndices.forEach(idx => {
      ...
      if (s > 0 && o > 0 && d > 0) {
        ...
        items.push({
          ...
          d,
          ap,
          action: riskData[`opt-action-${idx}`] as string,
        });
      }
    });
  ...
  return (
    <div className="flex flex-col h-full" style={{ width: RIGHT_PANEL_WIDTH }}>
      ...
      {selectedCell && selectedItems.length > 0 && (
        <div className="bg-green-50 border-b border-green-200 p-2 max-h-[200px] overflow-auto">
          <div className="text-[11px] font-bold text-green-700 mb-2">
            🔧 H→L 최적화 제안 ({selectedItems.length}건)
          </div>
          {selectedItems.map(item => {
            ...
            return (
              <div key={item.idx} className="bg-white rounded p-1.5 mb-1 text-[10px] border border-green-200">
