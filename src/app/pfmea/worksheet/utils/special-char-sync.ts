...
export function syncSCToMaster(
  ...
  let masterData: SpecialCharMaster[] = saved ? JSON.parse(saved) : [];
  ...
  if (scValue) {
    ...
    if (existingIdx === -1) {
      ...
      const newItem: SpecialCharMaster = {
        ...
        internalSymbol: 'SC',
        meaning: charType === 'product' ? '제품특성 SC' : '공정특성 SC',
        icon: '◆',
        color: '#e53935',
        partName: '',
        processName: '',
      ...
      console.log(`[특별특성] 마스터에 자동 추가: ${charType} "${charName}"`);
    } else {
      ...
      console.log(`[특별특성] 마스터 업데이트: ${charType} "${charName}"`);
    }
  } else {
    ...
    if (existingIdx !== -1) {
      ...
      console.log(`[특별특성] 마스터에서 연동 해제: ${charType} "${charName}"`);
    }
  }
...
export function autoSetSCForFailureMode(productCharName: string): boolean {
  ...
  if (sc) {
    console.log(`[특별특성] 고장형태 자동 SC 지정: 제품특성 "${productCharName}" → ${sc.customerSymbol}`);
    return true;
  }
  return false;
}
...
export function autoSetSCForFailureCause(processCharName: string): boolean {
  ...
  if (sc) {
    console.log(`[특별특성] 고장원인 자동 SC 지정: 공정특성 "${processCharName}" → ${sc.customerSymbol}`);
    return true;
  }
  return false;
}
