...
export function findLinkedFailureEffectsForRequirement(state: WorksheetState, requirementName: string): string[] {
  ...
  (state.l1?.failureScopes || []).forEach((scope: any) => {
...
export function findLinkedFailureModesForProductChar(state: WorksheetState, productCharName: string): string[] {
  ...
  (state.l2 || []).forEach((proc: any) => {
    (proc.failureModes || []).forEach((fm: any) => {
      ...
      const linkedChar = (proc.functions || []).flatMap((f: any) => f.productChars || [])
        .find((c: any) => c.id === fm.productCharId || c.name === productCharName);
...
export function findLinkedFailureCausesForProcessChar(state: WorksheetState, processCharName: string): string[] {
  ...
  (state.l2 || []).forEach((proc: any) => {
    (proc.failureCauses || []).forEach((fc: any) => {
      ...
      const linkedChar = (proc.l3 || []).flatMap((we: any) => 
        (we.functions || []).flatMap((f: any) => f.processChars || [])
      ).find((c: any) => c.id === fc.processCharId || c.name === processCharName);
