...
export function ConfirmStatusBar({ 
  ...
  return (
    <div className="flex gap-1">
      {isConfirmed ? (
        <span className={badgeStyles.confirmed}>✓ 확정됨({count})</span>
      ) : (
        <button type="button" onClick={onConfirm} className={badgeStyles.btnConfirm}>확정</button>
      )}
      <span className={missingCount > 0 ? badgeStyles.missing : badgeStyles.ok}>
        누락 {missingCount}건
      </span>
      {isConfirmed && (
        <button type="button" onClick={onEdit} className={badgeStyles.btnEdit}>수정</button>
      )}
    </div>
  );
}
