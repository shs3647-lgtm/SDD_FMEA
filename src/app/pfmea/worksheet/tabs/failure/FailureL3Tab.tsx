...
export default function FailureL3Tab({ state, setState, setDirty, saveToLocalStorage }: FailureTabProps) {
  ...
  // ========== DB 저장 데이터 vs 현재 입력 데이터 비교 ==========
  useEffect(() => {
    // localStorage에서 가장 최근 저장된 워크시트 찾기
    let savedData: string | null = null;
    let targetId = 'unknown';
    
    // 모든 pfmea_worksheet_ 키 확인
      ...
      if (key && key.startsWith('pfmea_worksheet_')) {
        ...
        if (data) {
          try {
            ...
            if (parsed.l2 && Array.isArray(parsed.l2)) {
              savedData = data;
              targetId = key.replace('pfmea_worksheet_', '');
              break;
            }
          } catch (e) {
            // 무시
    ...
    if (!savedData) {
      console.log('[고장원인 비교] 저장된 데이터 없음');
      return;
    }
    
    try {
      const parsed = JSON.parse(savedData);
      const savedL2 = parsed.l2 || [];
      
      // 저장된 failureCauses 추출
      const savedCauses: Array<{ procId: string; procName: string; causeId: string; causeName: string; processCharId: string }> = [];
      savedL2.forEach((proc: any) => {
        (proc.failureCauses || []).forEach((fc: any) => {
          savedCauses.push({
            ...
            causeId: fc.id,
            causeName: fc.name,
            processCharId: fc.processCharId || '없음'
          });
        });
      });
      
      // 현재 입력된 failureCauses 추출
      const currentCauses: Array<{ procId: string; procName: string; causeId: string; causeName: string; processCharId: string }> = [];
      ...
      console.log('═══════════════════════════════════════════════════════');
      console.log('🔍 [고장원인(FC)] DB 저장 데이터 vs 현재 입력 데이터 비교');
      console.log('   FMEA ID:', targetId);
      console.log('═══════════════════════════════════════════════════════');
      console.log('📦 DB에 저장된 데이터:');
      console.log('   - 총 고장원인:', savedCauses.length, '개');
      savedCauses.forEach((fc, idx) => {
        console.log(`   ${idx + 1}. [${fc.procName}] ${fc.causeName} (ID: ${fc.causeId}, processCharId: ${fc.processCharId})`);
      });
      console.log('');
      console.log('🖥️  현재 화면 입력 데이터:');
      console.log('   - 총 고장원인:', currentCauses.length, '개');
      currentCauses.forEach((fc, idx) => {
        console.log(`   ${idx + 1}. [${fc.procName}] ${fc.causeName} (ID: ${fc.causeId}, processCharId: ${fc.processCharId})`);
      });
      console.log('');
      
      // 비교 결과
      const savedIds = new Set(savedCauses.map(fc => fc.causeId));
      const currentIds = new Set(currentCauses.map(fc => fc.causeId));
      const savedNames = new Set(savedCauses.map(fc => `${fc.procId}_${fc.causeName}`));
      const currentNames = new Set(currentCauses.map(fc => `${fc.procId}_${fc.causeName}`));
      
      const idMatch = savedCauses.length === currentCauses.length && 
        savedCauses.every(fc => currentIds.has(fc.causeId));
      const nameMatch = savedCauses.length === currentCauses.length && 
        savedCauses.every(fc => currentNames.has(`${fc.procId}_${fc.causeName}`));
      
      console.log('✅ 비교 결과:');
      console.log('   - ID 일치:', idMatch ? '✅ 일치' : '❌ 불일치');
      console.log('   - 이름 일치:', nameMatch ? '✅ 일치' : '❌ 불일치');
      console.log('   - 저장된 개수:', savedCauses.length);
      console.log('   - 현재 개수:', currentCauses.length);
      
      if (!idMatch || !nameMatch) {
        console.error('❌❌❌ DB 저장 데이터와 현재 입력 데이터가 다릅니다! ❌❌❌');
        const missingInCurrent = savedCauses.filter(fc => !currentIds.has(fc.causeId));
        const extraInCurrent = currentCauses.filter(fc => !savedIds.has(fc.causeId));
        if (missingInCurrent.length > 0) {
          console.error('   - 현재 화면에 없는 항목:', missingInCurrent.map(fc => `${fc.procName}: ${fc.causeName}`));
        }
        if (extraInCurrent.length > 0) {
          console.error('   - DB에 없는 항목:', extraInCurrent.map(fc => `${fc.procName}: ${fc.causeName}`));
        }
      }
      console.log('═══════════════════════════════════════════════════════');
    } catch (e) {
      console.error('[고장원인 비교] 파싱 오류:', e);
    }
  }, [state.l2, state]);
