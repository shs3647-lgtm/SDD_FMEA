...
export default function RiskTabConfirmable({ 
  ...
  // 확정 상태
  const isConfirmed = (state as any).riskConfirmed || false;
  const isUpstreamConfirmed = (state as any).failureLinkConfirmed || false;
  
  // 리스크 데이터 - state에서 관리
  const riskData = (state as any).riskAnalysis || [];
  ...
  useEffect(() => {
    ...
    if (riskDataRef.current && dataKey !== riskDataRef.current) {
      console.log('[RiskTab] riskAnalysis 변경 감지, 자동 저장');
      saveToLocalStorage?.();
    }
    riskDataRef.current = dataKey;
  }, [riskData, saveToLocalStorage]);
  
  // 리스크 데이터 업데이트
  const updateRiskData = useCallback((id: string, field: keyof RiskData, value: any) => {
    setState(prev => {
      const analysis = (prev as any).riskAnalysis || [];
      ...
      if (existing) {
        ...
        return { ...prev, riskAnalysis: updated } as any;
      } else {
        // 신규 추가
        ...
        return { ...prev, riskAnalysis: [...analysis, newRisk] } as any;
      }
    });
    setDirty(true);
  }, [setState, setDirty]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    console.log('[RiskTab] 확정 버튼 클릭');
    
    const updateFn = (prev: any) => {
      const newState = { ...prev, riskConfirmed: true };
      console.log('[RiskTab] 확정 상태 업데이트:', newState.riskConfirmed);
      return newState;
    };
    ...
    setTimeout(() => {
      saveToLocalStorage?.();
      saveAtomicDB?.();
      console.log('[RiskTab] 확정 후 localStorage + DB 저장 완료');
    }, 50);
    ...
    <>
      <thead className={tw.thead}>
        ...
        <tr>
          ...
          <th className={`${tw.colHeader} ${tw.evaluationCell}`}>AP</th>
      ...
      <tbody>
        {failureCauses.length === 0 ? (
          ...
          failureCauses.map((fc: any, idx: number) => {
            ...
            return (
              <tr key={`risk-${fc.id}-${idx}`} className={`h-6 ${idx % 2 === 1 ? 'bg-gray-100' : 'bg-white'}`}>
                {/* 예방관리 */}
