...
export default function AllTabRenderer({ 
  ...
  const failureLinks = rawFailureLinks.map((link: any) => {
    ...
    return {
      ...
      fcWorkFunction: link.fcWorkFunction || fcToL3Map.get(normalizedFcId || link.fcId || '')?.workFunction || '',  // 작업요소 기능
      fcProcessChar: link.fcProcessChar || fcToL3Map.get(normalizedFcId || link.fcId || '')?.processChar || '',    // 공정특성
      // ★★★ FC 역전개 데이터 (고장원인 → 2L 구조분석) - fcToL3Map에서 fallback ★★★
      fcM4: link.fcM4 || fcToL3Map.get(normalizedFcId || link.fcId || '')?.m4 || '',          // 4M
      fcWorkElem: link.fcWorkElem || fcToL3Map.get(normalizedFcId || link.fcId || '')?.workElem || '',  // 작업요소
    };
  });
