...
export function useWorksheetState(): UseWorksheetStateReturn {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const cleanupKey = `dfmea_riskData_cleanup_v3_${fmeaId}`;
      if (!localStorage.getItem(cleanupKey)) {
        // 모든 관련 키 삭제
        localStorage.removeItem(`dfmea_riskData_${fmeaId}`);
        localStorage.removeItem(`dfmea_riskData_cleanup_v2_${fmeaId}`);
        localStorage.removeItem(`dfmea_riskData_cleanup_${fmeaId}`);
        localStorage.setItem(cleanupKey, 'done');
        console.log('[riskData v3] 잘못된 localStorage 데이터 완전 삭제 완료');
      }
      
      const riskDataStr = localStorage.getItem(`dfmea_riskData_${fmeaId}`);
      if (riskDataStr) {
        ...
        localStorage.setItem(`dfmea_riskData_${fmeaId}`, JSON.stringify(cleaned));
      }
    } catch (e) { /* ignore */ }
  ...
  const saveToLocalStorageOnly = useCallback(() => {
    ...
    try {
      ...
      try {
        const existingRaw = localStorage.getItem(`dfmea_worksheet_${targetId}`);
        if (existingRaw) {
  ...
  const saveToLocalStorage = useCallback(() => {
    ...
    try {
      ...
      const savedData = localStorage.getItem(`dfmea_worksheet_${targetId}`);
      if (savedData) {
        try {
    ...
    if (causesKey !== failureCausesRef.current) {
      ...
      failureCausesSaveTimeoutRef.current = setTimeout(() => {
        ...
        requestAnimationFrame(() => {
          const targetId = selectedFmeaId || currentFmea?.id;
          if (targetId) {
            const savedKey = `dfmea_worksheet_${targetId}`;
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(currentRiskData) !== JSON.stringify(riskDataRef.current)) {
      ...
      // ✅ 별도 키로 직접 저장 (확실한 저장)
      localStorage.setItem(`dfmea_riskData_${targetId}`, JSON.stringify(currentRiskData));
      console.log('[저장완료] riskData 저장:', `dfmea_riskData_${targetId}`);
  ...
  useEffect(() => {
    ...
    if (state.tab !== tabRef.current) {
      ...
      // ✅ 별도 키로 직접 저장 (백업용)
      localStorage.setItem(`dfmea_tab_${targetId}`, state.tab);
    ...
    if (stored) {
      try {
        ...
        if (selectedFmeaId) {
          ...
          router.push(`/dfmea/worksheet?id=${projects[0].id}`);
        }
      } catch (e) { 
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        localStorage.setItem(`dfmea_worksheet_${selectedFmeaId}`, JSON.stringify(worksheetData));
        
        // 5. URL에서 상속 파라미터 제거 (새로고침 시 중복 상속 방지)
        const newUrl = `/dfmea/worksheet?id=${selectedFmeaId}`;
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem(migrationKey)) {
      ...
      const keys = ['dfmea_worksheet_', 'dfmea_tab_', 'dfmea_riskData_'];
      keys.forEach(prefix => {
    ...
    let projectL1Name = '';
    try {
      const projectsData = localStorage.getItem('dfmea-projects');
      if (projectsData) {
    ...
    (async () => {
      ...
      const urlTabParam = typeof window !== 'undefined' 
        ...
        : null;
      let legacyTab = urlTabParam || localStorage.getItem(`dfmea_tab_${selectedFmeaId}`) || 'structure';
      ...
      if (!hasDbResponse) {
        const legacyKeys = [
          `dfmea_worksheet_${selectedFmeaId}`,
          `fmea-worksheet-${selectedFmeaId}`,
          `dfmea_atomic_${selectedFmeaId}`, // 원자성 DB 백업
        ];
      ...
      const legacyKeys2 = [`dfmea_worksheet_${selectedFmeaId}`, `fmea-worksheet-${selectedFmeaId}`];
      // ✅ 2026-01-12: URL 탭 우선 → localStorage 탭 → 기본값
      const urlTabParam2 = typeof window !== 'undefined' 
        ? new URLSearchParams(window.location.search).get('tab') 
        : null;
      let legacyTab2 = urlTabParam2 || localStorage.getItem(`dfmea_tab_${selectedFmeaId}`) || 'structure';
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          ...
          if (urlParams.get('reset-fm') === 'true') {
            ...
            localStorage.setItem(`dfmea_worksheet_${selectedFmeaId}`, JSON.stringify(parsed));
        ...
        if (parsed.l1 && parsed.l2 && (hasValidL1 || hasValidL2)) {
          ...
          if (!migratedL1.name || migratedL1.name.trim() === '') {
            const atomicKey = `dfmea_atomic_${selectedFmeaId}`;
