...
export function buildFailureAnalyses(db: FMEAWorksheetDB): FailureAnalysis[] {
  ...
  if (!failureLinks || failureLinks.length === 0) {
    console.log('[고장분석 빌더] 고장연결 없음, 빈 배열 반환');
    return [];
  }
  
  console.log('[고장분석 빌더] 시작 - 총 연결 수:', failureLinks.length);
  ...
  failureLinks.forEach((link: FailureLink) => {
    ...
    if (!fm) {
      console.warn('[고장분석 빌더] FM 누락:', link.fmId);
      return;
    }
  ...
  console.log('[고장분석 빌더] 완료 - 생성된 분석 수:', failureAnalyses.length);
