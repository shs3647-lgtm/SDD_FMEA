  ...
  return (
    <>
      <thead className={tw.thead}>
        ...
        {/* 2행: 서브그룹 */}
        <tr>
          <th colSpan={1} className={`${subHeader} ${tw.structureHeader}`}>1. 제품명</th>
          <th colSpan={1} className={`${subHeader} ${tw.structureHeader}`}>2. A'SSY명</th>
          <th colSpan={2} className={`${subHeader} ${tw.structureHeader}`}>3. 작업 요소명</th>
          <th colSpan={3} className={`${subHeader} ${tw.functionHeader}`}>1. 제품 기능/요구사항</th>
          <th colSpan={2} className={`${subHeader} ${tw.functionHeader}`}>2. A'SSY 기능 및 제품 특성</th>
          <th colSpan={3} className={`${subHeader} ${tw.functionHeader}`}>3. 부품 기능 및 부품 특성</th>
        ...
        {/* 3행: 컬럼명 */}
        <tr>
          <th className={`${baseHeader} ${tw.structureCell}`}>제품명</th>
          <th className={`${baseHeader} ${tw.structureCell}`}>A'SSY명</th>
          {/* DFMEA: 4M 제거됨 */}
          <th className={`${baseHeader} ${tw.structureCell}`}>부품 또는 특성</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>구분</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>제품 기능</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>요구사항</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>A'SSY 기능</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>제품 특성</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>부품 또는 특성</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>부품 기능</th>
          <th className={`${baseHeader} ${tw.functionCell}`}>부품 특성</th>
          ...
          <th className={`${baseHeader} ${tw.failureCell}`}>부품 또는 특성</th>
          ...
          rows.map((row, idx) => {
            ...
            return (
              <tr key={`risk-full-${row.l1Id}-${row.l2Id}-${row.l3Id}-${idx}`} className={`h-5 ${zebra}`}>
                ...
                {l2Spans[idx] > 0 && (
                  <td rowSpan={l2Spans[idx]} className={`${baseCell} ${tw.structureCell}`}>{row.l2Name}</td>
                )}
                {/* DFMEA: m4 제거됨 */}
