 ...
 * @description 기능분석(3단계) 셀 렌더링 - 구분, 제품 기능, 요구사항, A'SSY 기능, 제품 특성, 부품 기능, 부품 특성 (DFMEA)
...
export function FunctionCellRenderer({
  ...
  // ★ FE 기반 역전개 (구분, 제품 기능, 요구사항) (DFMEA)
  const feFunctionCols = ['구분', '제품 기능', '요구사항'];
  if (feFunctionCols.includes(col.name)) {
    ...
    if (rowInFM === 0 || !isInMergedRange('fe')) {
      ...
      else if (col.name === '제품 기능') value = row.feFunctionName || '';
  ...
  // ★ FM 기반 역전개 (A'SSY 기능, 제품 특성) (DFMEA)
  const fmFunctionCols = ['A\'SSY 기능', '제품 특성'];
  if (fmFunctionCols.includes(col.name)) {
    if (row.isFirstRow) {
      let value = '';
      if (col.name === 'A\'SSY 기능') value = fmGroup.fmProcessFunction || '';
      else if (col.name === '제품 특성') value = fmGroup.fmProductChar || '';
  ...
  // ★ FC 기반 역전개 (부품 기능, 부품 특성) (DFMEA)
  const fcFunctionCols = ['부품 기능', '부품 특성'];
  if (fcFunctionCols.includes(col.name)) {
    // 누적 범위 체크: 이전 행의 fcRowSpan 범위 안에 있으면 렌더링하지 않음
    if (rowInFM === 0 || !isInMergedRange('fc')) {
      let value = '';
      if (col.name === '부품 기능') value = row.fcWorkFunction || '';
      else if (col.name === '부품 특성') value = row.fcProcessChar || '';
