...
export default function AllTabAtomic({ fmeaId, visibleSteps = [2, 3, 4, 5, 6], setState, onNoData }: AllTabAtomicProps) {
  ...
  // A'SSY별 그룹핑 (rowSpan 계산용) - 구조/기능분석용 (DFMEA)
  const processedRows = useMemo(() => {
    if (rows.length === 0) return [];
    
    // A'SSY별, FM별 그룹핑 (DFMEA)
    ...
    rows.forEach((row, idx) => {
      const isNewProcess = row.l2StructName !== prevProcess; // DFMEA: l2StructNo → l2StructName
      const isNewFm = row.fmId !== prevFm;
      const isNewFe = row.feId !== prevFe;
      
      // 새 A'SSY이면 이전 A'SSY의 rowSpan 계산 (DFMEA)
      ...
      if (isNewProcess) {
        processStartIdx = idx;
        prevProcess = row.l2StructName; // DFMEA: l2StructNo → l2StructName
      }
      if (isNewFm) {
        fmStartIdx = idx;
        prevFm = row.fmId;
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: getOptimizedWidth }}>
        <colgroup>
          ...
          {visibleSteps.includes(4) && (
            <>
              ...
              <col style={{ width: '100px' }} /> {/* 부품 또는 특성 (DFMEA) */}
            </>
          )}
          {/* 리스크분석 8열 */}
        ...
        <thead>
          ...
          <tr>
            {visibleSteps.includes(2) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.structure.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>1.제품명</th>
                <th style={stickyHeaderCellStyle(COLORS.structure.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>2.A'SSY명</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.structure.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>3.작업<br/>요소명</th>
              </>
            )}
            {visibleSteps.includes(3) && (
              <>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>1.제품 기능<br/>/요구사항</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>2.A'SSY 기능<br/>및 제품 특성</th>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>3.부품 기능<br/>및 부품 특성</th>
              </>
            )}
            {visibleSteps.includes(4) && (
              <>
                <th colSpan={3} style={stickyHeaderCellStyle('#f9a825', HEADER_ROW_H, '#333', 59, '9px', 600)}>1.자사/고객/사용자<br/>고장영향(FE)</th>
                <th colSpan={2} style={stickyHeaderCellStyle('#7e57c2', HEADER_ROW_H, '#fff', 59, '9px', 600)}>2.A'SSY<br/>고장형태(FM)</th>
                <th colSpan={2} style={stickyHeaderCellStyle('#66bb6a', HEADER_ROW_H, '#fff', 59, '9px', 600)}>3.부품<br/>고장원인(FC)</th>
              </>
            )}
            {visibleSteps.includes(5) && (
              <>
          ...
          <tr>
            {visibleSteps.includes(2) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.structure.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>제품명</th>
                <th style={stickyHeaderCellStyle(COLORS.structure.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>A'SSY명</th>
                {/* DFMEA: 4M 제거됨 */}
            ...
            {visibleSteps.includes(3) && (
              <>
                ...
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>제품<br/>기능</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>요구<br/>사항</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>A'SSY<br/>기능</th>
                ...
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>부품<br/>기능</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>부품<br/>특성</th>
              </>
            )}
            {visibleSteps.includes(4) && (
              <>
                    ...
                    <>
                      {rowIdx === 0 && (
                        <>
                          ...
                          <td rowSpan={totalRows} style={cellStyle(zebra.structure, 'center')}>{structRow?.l2StructName || ''}</td> {/* DFMEA: l2StructNo 제거됨 */}
                        </>
                      )}
                      {/* DFMEA: l3M4 제거됨 */}
