 ...
 * @description A'SSY(L2) 기능 분석 - 3행 헤더 구조 (L1과 동일한 패턴) (DFMEA)
 * 
...
export default function FunctionL2Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB }: FunctionTabProps) {
  ...
  // ✅ 2L COUNT 계산 (A'SSY, A'SSY 기능, 제품 특성) (DFMEA)
  ...
  const handleConfirm = useCallback(() => {
    ...
    alert('✅ 2L A\'SSY 기능분석이 확정되었습니다. (DFMEA)');
  ...
  // A'SSY 기능 인라인 편집 핸들러 (더블클릭) (DFMEA)
        ...
        newState.l2 = newState.l2.map((proc: any) => {
          ...
          for (let i = startIdx; i < selectedValues.length; i++) {
            ...
            if (!existingNames.has(val)) {
              // ✅ 자동연결: 다른 A'SSY에서 동일 기능에 연결된 제품 특성 찾기 (DFMEA)
      ...
      <table className="w-full border-collapse table-fixed">
        ...
        <thead className="sticky top-0 z-20 bg-white border-b-2 border-black">
          ...
          <tr>
            ...
            <th colSpan={3} className="bg-[#388e3c] text-white border border-[#ccc] p-2 text-xs font-extrabold text-center">
              <div className="flex items-center justify-center gap-5">
                <span>3단계 : 2L A'SSY 기능분석 (DFMEA)</span>
          ...
          <tr className="bg-[#e8f5e9]">
            <th className="bg-[#e3f2fd] border border-[#ccc] p-1.5 text-xs font-semibold">
              A'SSY명<span className={`font-bold ${processCount > 0 ? 'text-green-700' : 'text-red-500'}`}>({processCount})</span>
            </th>
            <th className="bg-[#c8e6c9] border border-[#ccc] p-1.5 text-xs font-semibold">
              A'SSY 기능<span className={`font-bold ${l2FunctionCount > 0 ? 'text-green-700' : 'text-red-500'}`}>({l2FunctionCount})</span>
            </th>
        ...
        <tbody>
          {state.l2.length === 0 ? (
              ...
              return (
                <tr>
                  <td className="border border-[#ccc] p-2.5 text-center font-semibold" style={{ background: zebra.structure }}>
                    (구조분석에서 A'SSY 추가) (DFMEA)
                  </td>
                  <td className={cellP0} style={{ background: zebra.function }}>
                    <SelectableCell value="" placeholder="A'SSY 기능 선택" bgColor={zebra.function} onClick={() => {}} />
                  </td>
            ...
            let globalRowIdx = 0;
            let funcCounter = 0; // ✅ A'SSY 기능 블록 인덱스 (DFMEA)
                ...
                if (meaningfulFuncs.length === 0) {
                  ...
                  return (
                    <tr key={proc.id}>
                      ...
                      <td className={cellP0} style={{ background: zebra.function }}>
                        <SelectableCell value="" placeholder="A'SSY 기능 선택" bgColor={zebra.function} onClick={() => handleCellClick({ type: 'l2Function', procId: proc.id, title: 'A\'SSY 기능 선택', itemCode: 'A3' })} />
                      </td>
                      <td className={cellP0} style={{ background: zebra.failure }}>
                        <SelectableCell value="" placeholder="제품특성 선택" bgColor={zebra.failure} textColor={'#e65100'} onClick={() => {}} />
                      </td>
