  ...
  return (
    <>
      ...
      <thead className="sticky top-0 z-20 bg-white border-b-2 border-black">
        ...
        {/* 2행: 서브그룹 */}
        <tr>
          <th colSpan={1} style={subH(COLORS.structure.header)}>1. 제품명</th>
          <th colSpan={1} style={subH(COLORS.structure.header)}>2. A'SSY명</th>
        ...
        {/* 3행: 컬럼명 */}
        <tr>
          <th style={colH(COLORS.structure.cell)}>제품명</th>
          <th style={colH(COLORS.structure.cell)}>A'SSY명</th>
          {/* DFMEA: 4M 제거됨 */}
          <th style={colH(COLORS.structure.cell)}>부품 또는 특성</th>
          ...
          rows.map((row, idx) => {
            ...
            return (
              <tr key={`opt-full-${row.l1Id}-${row.l2Id}-${row.l3Id}-${idx}`} className={`h-[22px] ${zebraBg}`}>
                ...
                {l2Spans[idx] > 0 && <td rowSpan={l2Spans[idx]} style={dc(COLORS.structure.cell)}>{row.l2Name}</td>}
                {/* DFMEA: m4 제거됨 */}
