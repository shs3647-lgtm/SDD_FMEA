    ...
    <div className="min-h-screen bg-[#f0f0f0] p-3 font-[Malgun_Gothic]">
        ...
        <table className="w-full border-collapse text-xs">
          <tbody>
            <tr style={{ height: '32px' }}>
              <td className={headerCell} style={{ width: '8%' }}>고객사</td>
              <td className={inputCell} style={{ width: '12%' }}>
                <input type="text" className="w-full h-7 px-2 text-xs border-0 bg-transparent focus:outline-none" readOnly />
              </td>
              <td className={headerCell} style={{ width: '8%' }}>공장</td>
              <td className={inputCell} style={{ width: '12%' }}>
                <input type="text" className="w-full h-7 px-2 text-xs border-0 bg-transparent focus:outline-none" readOnly />
              </td>
              <td className={headerCell} style={{ width: '8%' }}>프로젝트</td>
              ...
              <td className={headerCell} style={{ width: '8%' }}>품명</td>
              <td className={inputCell} style={{ width: '12%' }}>
                <input type="text" className="w-full h-7 px-2 text-xs border-0 bg-transparent focus:outline-none" readOnly />
              </td>
              <td className={headerCell} style={{ width: '8%' }}>품번</td>
              <td className={inputCell} style={{ width: '12%' }}>
                <input type="text" className="w-full h-7 px-2 text-xs border-0 bg-transparent focus:outline-none" readOnly />
              </td>
            </tr>
      ...
      <div className="bg-white rounded border border-gray-300 mb-3">
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className={headerCell} style={{ width: '5%' }}>No</th>
              <th className={headerCell} style={{ width: '15%' }}>프로젝트</th>
              <th className={headerCell} style={{ width: '8%' }}>개정번호</th>
              <th className={headerCell} style={{ width: '10%' }}>개정일자</th>
              <th className={headerCell} style={{ width: '15%' }}>개정사유</th>
              <th className={headerCell} style={{ width: '20%' }}>개정내용</th>
              <th className={headerCell} style={{ width: '8%' }}>작성자</th>
              <th className={headerCell} style={{ width: '8%' }}>승인자</th>
              <th className={headerCell} style={{ width: '8%' }}>상태</th>
            </tr>
          </thead>
          <tbody>
            {filteredRevisions.map((rev, idx) => (
              <tr key={rev.id} className={idx % 2 === 0 ? 'bg-[#e3f2fd]' : 'bg-white'} style={{ height: '28px' }}>
            ...
            {Array.from({ length: emptyRevisionRows }).map((_, idx) => (
              <tr key={`empty-rev-${idx}`} className={`${(filteredRevisions.length + idx) % 2 === 0 ? 'bg-[#e3f2fd]' : 'bg-white'}`} style={{ height: '28px' }}>
      ...
      <div className="bg-white rounded border border-gray-300">
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className={headerCell} style={{ width: '5%' }}>No</th>
              <th className={headerCell} style={{ width: '10%' }}>회의일자</th>
              <th className={headerCell} style={{ width: '15%' }}>회의제목</th>
              <th className={headerCell} style={{ width: '15%' }}>참석자</th>
              <th className={headerCell} style={{ width: '15%' }}>안건</th>
              <th className={headerCell} style={{ width: '15%' }}>결정사항</th>
              <th className={headerCell} style={{ width: '12%' }}>실행항목</th>
              <th className={headerCell} style={{ width: '8%' }}>기록자</th>
            </tr>
          </thead>
          <tbody>
            {filteredMeetings.map((mtg, idx) => (
              <tr key={mtg.id} className={idx % 2 === 0 ? 'bg-[#e3f2fd]' : 'bg-white'} style={{ height: '28px' }}>
                <td className="border border-gray-400 px-2 py-1 text-center align-middle font-bold text-[#00587a]">{idx + 1}</td>
                <td className="border border-gray-400 px-1 py-0.5">
                  <input 
                    type="date" 
            ...
            {Array.from({ length: emptyMeetingRows }).map((_, idx) => (
              <tr key={`empty-mtg-${idx}`} className={`${(filteredMeetings.length + idx) % 2 === 0 ? 'bg-[#e3f2fd]' : 'bg-white'}`} style={{ height: '28px' }}>
