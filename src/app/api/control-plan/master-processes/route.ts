...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!activeDataset) {
      console.log('⚠️ 활성화된 CP Master Dataset 없음');
    ...
    console.log(`✅ CP Master 공정 ${processes.length}개 반환 (dataset: ${activeDataset.name})`);
    ...
    console.error('CP 공정 조회 오류:', error.message);
    return NextResponse.json({ 
      success: false, 
      processes: [],
      error: error.message 
